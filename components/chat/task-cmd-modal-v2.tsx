      ...
      const res = await fetch("/api/ai/revise-task-draft", {
        ...
        body: JSON.stringify({
          ...
          current: {
            ...
            ownerName: users.find((u) => u.id === v.ownerId)?.name ?? null,
            ...
            */}
            <div className="sm:col-span-2">
              ...
              <Select value={ownerId || ""} onValueChange={(v) => setValue("ownerId", v ?? "", { shouldDirty: true })}>
                <SelectTrigger id="task-owner-trigger" className={`h-9 w-full text-[13px] ${errors.ownerId ? "border-destructive" : ""} ${glowFields.has("ownerId") ? "ai-fill-glow" : ""}`}>
