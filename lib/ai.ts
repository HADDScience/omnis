...
export async function answerWithOmnis(
  ...
  // 이 블록에는 업무만이 아니라 지식재산권·CRM 현황도 함께 들어온다.
  // "업무 현황" 이라고만 소개하면 모델이 나머지를 자기 소관이 아니라고 보고
  // "관련 내용을 찾지 못했습니다" 로 답한다 — 자료는 줬는데 안 읽는 셈이다.
  const overviewSection = taskOverview
    ? `\n\n[현황 자료 — 업무 · 지식재산권 · 고객/견적/재고의 현재 전량]\n${taskOverview}`
    : ""
...
- [참고 자료]를 근거로 쓰면 문장 끝에 [1], [2]처럼 번호를 표기하세요. [현황 자료]를 근거로 할 때는 번호 표기가 필요 없습니다.
- [현황 자료]는 검색 결과가 아니라 **현재 전량**입니다. 업무·마감일·지연·진행 상태, 상표·특허, 원료 재고·견적·샘플요청·거래 기관을 묻는 질문은 이 자료를 근거로 하나도 빠뜨리지 말고 모두 답하세요.
- 재고·금액·건수는 [현황 자료]에 적힌 값을 그대로 쓰세요. 직접 더하거나 어림하지 마세요.
