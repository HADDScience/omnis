...
**아직 운영에서 해결을 확인하지 않았다.** 로컬에서는 1단계(이유 표시)와 2단계(NAS 파일 연결)가 동작한다.
운영 배포 뒤 확인할 것과 남은 것은 맨 아래.
...
| 무엇 | 왜 남았나 |
|---|---|
| **큰 파일 내려받기가 Vercel 함수 수명 안에 끝나는가** | NAS → Vercel 함수 → 브라우저로 흐른다. 로컬(맥 → NAS)에서 185MB 파일의 앞 5MB 가 2.6초 — 같은 속도면 전체에 90초 남짓이다. 운영에서 재지 않았다 |
| NAS 에 없는 큰 파일 | 사람이 NAS 에 먼저 올려야 한다. 조각 업로드 · NAS 수신기는 하지 않았다 |
| NAS 파일이 옮겨지거나 지워지면 링크가 깨진다 | 사본을 두지 않는 선택의 대가. 열 때 「찾지 못했습니다」 |
| GitHub 이슈 | `GITHUB_ISSUE_TOKEN` 이 운영에 없으면 메일만 나간다 |
