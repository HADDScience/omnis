...
멘션 감지 시
  ...
  4. 기존 체크리스트 삭제 → 새로 생성
  → 체크리스트 전부 완료면 자동 DONE 이 아니라 담당자에게 완료 확인 요청
  → 시스템 메시지(🤖) 를 DB 에 저장
```
...
화면 경로는 2026-09-15(PR #16)부터 재구성을 응답 뒤(`after()`)에 돌린다. 쓰기 경로 · 끝 기록 ·
완료 업무 참조 규칙의 정본은 [chat-post.md](chat-post.md) 다.
...
폴링은 모든 사내 사이트가 같이 쓰는 Vercel 요청 한도를 먹는다. 새 폴링 · 간격 변경은
[request-budget.md](request-budget.md) 의 규칙(`useVisibleInterval`)을 따른다.
