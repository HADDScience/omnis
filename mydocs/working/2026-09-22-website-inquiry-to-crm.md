...
# 3차 — 보관기간 파기를 실제로 돌려 봤다 (2026-09-23)

1차·2차에서 「돌려 보지 않았다」 고 남겨 둔 것을 채웠다. 3년 지난 행을 심어 `--apply` 를 돌렸다.
**운영 데이터로 하지 않았다** — 심은 행만 쓰고 끝나고 지웠다.

## 문의 3년 파기 — 다섯 갈래

심은 것: `createdAt` 이 3년+1일 전인 NEW · REJECTED · SPAM · ACCEPTED 각 1건,
그리고 경계로 3년−1일 전 NEW 1건. ACCEPTED 에는 실제 기관·담당자·견적을 이어 두었다.

미리보기:

```
기준: 2023-09-23 이전 접수분 (3년)
  삭제 대상 (NEW·REJECTED·SPAM): 3건
  개인정보만 비울 대상 (ACCEPTED): 1건
아무것도 바꾸지 않았습니다.
```
...
[ACCEPTED] (파기됨)  (2023-09-23)
   organization=null email="" phone=null
   message="(보관기간 경과로 파기)" ip=null ua=null
   reviewedAt=2023-09-23 reviewNote=null org=있음 contact=있음 quote=있음
[NEW] 경계-NEW  (2023-09-25)
   organization="파기시험 소속" email="edge-new@example.com" phone="010-1111-2222"
   message="3년 하루 모자란 NEW" ip="203.0.113.77" ua="PurgeTest/1.0"

총 2 건 | 기관 27 | 견적 14
```

- NEW · REJECTED · SPAM 셋은 **행째 사라졌다**
- ACCEPTED 는 행이 남고 개인정보만 비었다. `status`·`reviewedAt`·`orgId`·`contactId`·`quoteId`
  는 그대로다 — 이어진 기관·견적도 그대로(27곳·14건)
- **경계(3년−1일)는 건드리지 않았다.** 하루 차이로 안 지워야 할 것이 지워지지 않는다

### `reviewNote` 도 비우기로 바꿨다

처음 설계에는 없었다. 담당자가 자유롭게 쓰는 칸이라 「전화로 통화, 김OO 교수」 처럼
**방문자 개인정보가 들어갈 수 있다.** 방침이 3년 뒤 파기를 약속하는데 그 칸만 남으면
약속이 절반만 지켜진다. 「왜 그렇게 처리했는가」 는 이어진 견적·샘플요청의 `note` 에 남고,
「사람이 언제 처리했는가」 는 `status`·`reviewedAt` 이 지킨다.

### 빈 이메일이 중복 판정·레이트리밋을 흔드는가 — 안 흔든다

파기된 행은 `email = ""` 이 된다. 사이트 세션이 그게 나중에 문제가 되지 않는지 물어서 재 봤다:

```
빈 이메일 + 같은 본문 → 중복으로 잡히나?  안 잡힘
빈 이메일 레이트리밋:                      안 막힘
빈 이메일 접수 가능?                       불가 — 이메일 형식이 아닙니다
```

세 겹으로 막힌다. 중복 판정은 5분 창이라 3년 된 행이 애초에 후보가 아니고,
레이트리밋도 같은 창을 본다. 그리고 `inquiryIntakeSchema` 가 빈 이메일을 400 으로 막아
`""` 인 행이 접수로 생길 길이 없다 — 파기가 만드는 것뿐이다.

## 방문 통계 400일 파기 — 새로 만들었다

`scripts/purge-website-visits.ts`.

400일은 내가 정한 값이 아니라 `mydocs/plans/2026-09-22-website-visit-stats.md:75` 가 근거와
함께 정한 값이다 — 「작년 같은 달과 비교하려면 1년 + 여유가 필요하다」. 사이트 방침도 그
숫자를 옮겨 적었다. 계획서에 `DELETE ... interval '400 days'` 까지 적혀 있고 자동이 아니었을 뿐이다.

**행째 지운다.** 문의와 다른 점이다 — 방문 기록에는 지킬 「사람이 승인했다」 가 없다.
미리 말아 둔 집계 표도 없다(`lib/website-visits.ts` 의 통계는 전부 원시 행에서 그때그때 센다).
그래서 행을 지우면 그 기간의 숫자도 함께 사라진다. 그것이 보관기간의 뜻이다.
여기에는 IP 도 UA 도 쿠키도 없고 하루짜리 해시뿐이다.

심은 것: 500일 전 · 401일 전 · 399일 전(경계) · 1일 전.

```
######## 미리보기 ########
기준: 2025-08-19 이전 방문 (400일)
  전체 4건 · 가장 오래된 방문 2025-05-11
  삭제 대상: 2건
아무것도 바꾸지 않았습니다.
...
삭제 2건 · 남은 2건
  ...
  /ko/news/d399  2025-08-20  (399일 전)
  /ko/news/d001  2026-09-22  (1일 전)
```

401·500일은 사라지고 **399일은 남았다.**

미리보기에 「가장 오래된 방문」 을 함께 찍는다 — 「0건」 이 기준이 맞아서인지 데이터가 아예
없어서인지 구분되지 않으면 스크립트가 도는지도 알 수 없다.
...
```
남은 — 문의 0 | 방문 0 | 기관 26 | 담당자 25 | 견적 13
```

## 아직 안 한 것

- **운영에서는 돌리지 않았다.** 두 스크립트 모두 로컬에서만 태웠다. 운영에는 아직 3년·400일
  지난 데이터가 없어 지울 것도 없다
- 크론. 한 해에 몇 번 돌릴 일이라 붙이지 않았다
