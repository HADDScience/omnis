...
test.describe("시연용 데모 배너 + 테스트 계정 카드", () => {
  ...
  test("로그인 페이지에 floating 배너가 나타난다", async ({ page }) => {
    ...
    await expect(
      page.getByRole("status", { name: "시연용 데모 사이트" })
    ).toBeVisible()
    await expect(
      page.getByText("시연용 데모 사이트 — 모든 데이터는 익명 더미입니다.", {
        exact: true,
      })
    ).toBeVisible()
  })

  test("로그인 페이지에 5인 quick-login 버튼이 모두 보인다", async ({
    page,
  }) => {
    await page.goto("/login")
  ...
  test("로그인 후 대시보드에서도 배너가 유지된다 (sticky/fixed)", async ({
    page,
  }) => {
    await login(page, "팀장")
    await expect(
  ...
  test("배너가 헤더(Sidebar)와 겹치지 않는다 (회귀 방지)", async ({ page }) => {
    await login(page, "팀장")
    const banner = page
      .getByRole("status", { name: "시연용 데모 사이트" })
      .first()
    const sidebarLogo = page
      .getByRole("link", { name: /Omnis.*HADDScience/ })
      .first()
